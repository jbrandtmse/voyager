...
1. **Reconstruct the file list from disk** — `git status --short` from the repo root; collect modified + new files that match the story spec's `Files to Modify` table. The reconstructed list serves as whichever completion-message field the silent agent should have populated:
   - **Dev silence** → `FILES_MODIFIED` (all changed files matching the story's expected file list).
   - **QA silence** → `TESTS_ADDED` (filter the changed-file list to whatever test-file naming convention the project uses — e.g., `*.test.ts`, `test_*.py`, `*_test.go`, `*Spec.scala`, files under `tests/` or `spec/`, etc. — plus any files matching the story's `Tests / Subtasks` paths).
   - **Code-reviewer silence** → typically no new source files (the cr agent edits the story file's `Review Findings` section but rarely changes source). If files DID change, they are the cr's auto-resolved HIGH/MED fixes; capture them so the commit stage includes them.
...
3. **Run the verification the silent agent should have run** — invoke the project's test suite plus any type-check, lint, or build checks it uses. The exact commands vary per project; consult the project's task runner (e.g., `justfile`, `package.json` `scripts`, `Makefile`, `pyproject.toml`, `Cargo.toml`, `go.mod` + a `Makefile`, README quick-start, or the project's BMAD build-rules file under `_bmad/custom/`) for the canonical invocations. Stage-specific guidance:
   - **Dev silence** → run the full project test suite + whatever type/lint/build checks the dev agent normally runs as part of its skill.
   - **QA silence** → run the same suite, and additionally confirm that the new test files identified in step 1 actually execute as part of the run (not excluded by an ignore file, not skipped by a tag or marker, not in a `slow`/`integration` opt-in bucket the default suite skips). The whole point of QA's work is *new* tests that *run*; verify both.
   - **Code-reviewer silence** → read the story file's `Review Findings` section if the agent populated it; if not, the lead reads the diff and runs the code-review skill itself directly via the `Skill` tool (lead-driven; same pattern as `/bmad-retrospective`).

   On verification failure, surface to the user — do NOT silently absorb a verification regression.
