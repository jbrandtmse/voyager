# Epic Development Cycle Workflow — v2
...
v2 incorporates everything learned through Epic 1 + Epic 2's first stories, including the load-bearing fix for the agent-silence failure mode (`on_complete` rather than `persistent_facts` for the structured-completion handshake — see Part 1's schema discussion for the full reasoning, including why `activation_steps_append` also fails for this purpose).

---

## Pre-flight: Agent Teams must be enabled

**Requires Claude Code v2.1.32 or later.** This workflow depends on the experimental Agent Teams feature; without it the lead cannot spawn named teammates via `Agent`, address them via `SendMessage`, or use `TeamCreate` / `TeamDelete`.

### Step 1 — Check whether it's already enabled

Confirm that the `Agent` tool accepts a `name:` parameter and that `SendMessage`, `TeamCreate`, and `TeamDelete` are available in this session. If yes, skip to "Construction overview."

### Step 2 — Enable Agent Teams (if not already on)

Either ask Claude Code to use the `update-config` skill to add this to `settings.json`:
...
…or edit `~/.claude/settings.json` (or `.claude/settings.json` for project-local scope) manually.

### Step 3 — Restart Claude Code

Environment variables are read at startup; fully exit and relaunch.

### Step 4 — Re-run this prompt

After relaunching, re-run the v2 construction. The pre-flight will pass.

---

## Construction overview

Once Agent Teams is on, the reader (a Claude Code session executing this doc) does the following in order:

1. Pick the project's short name (e.g., `voyager`, `loanapp`, `engine`). Substitute it for every `<project>` placeholder below. Throughout Part 1, this name appears in file names and persistent-fact paths; keep it consistent.
2. Create the BMAD customization files in Part 1, in order — the rules file first, then each `.toml`. Each `.toml` references the rules file by path.
...
5. (Optional) Delete this v2 design doc — the slash command + customizations are now standalone.

---

## Part 1: BMAD skill customizations

The `/epic-cycle` workflow has gates that depend on each invoked BMAD skill behaving a certain way: ending with a structured `STATUS: completed` envelope, enforcing integration ACs at story-creation time, treating ADR violations as HIGH severity, pausing on NFR tripwires, and so on. The workflow describes these requirements as "the skill should ideally…" — these customization files are what makes "ideally" into "actually."

**Critical schema detail.** BMAD's `[workflow]` customization surface has four fields (per `bmad-workflow-builder/references/skill-quality-principles.md` and the base `customize.toml` for every skill):

- `activation_steps_prepend` — runs BEFORE the standard activation (config load, greet). For pre-flight loads or compliance checks. *Wrong place for completion behavior.*
- `activation_steps_append` — runs AFTER greet but BEFORE the workflow's main work begins. For context-heavy setup. *Wrong place for completion behavior.*
- `persistent_facts` — static context loaded on activation; the LLM sees it throughout the run but it does not guarantee any specific runtime behavior. *Right place for rule definitions; wrong place for behavior that must fire at a specific moment.*
- `on_complete` — fires when the workflow reaches its terminal stage, AFTER the main output has been delivered. *Right place for the structured-completion handshake and any final-stage verification.*

The structured-completion handshake MUST be in `on_complete`. Putting it in `persistent_facts` produces the empirically-observed ~10–12% spawn-silence failure mode (the rule is visible to the LLM throughout, but the skill body's natural ending can finish without acting on it). Putting it in `activation_steps_append` also fails — those steps run during setup, before the actual work, so they don't fire at completion either.

**One more catch (the silent-no-op trap):** custom overrides only take effect for fields the skill's `SKILL.md` actually references via `{workflow.<field>}`. The base `customize.toml` for each BMAD skill the workflow invokes declares `on_complete = ""` and the SKILL.md reads it at the terminal stage — so `on_complete` overrides do fire. Verify this by checking each skill's `customize.toml`: the comment above `on_complete` will name the specific completion-stage step (e.g., "executed when the workflow reaches Step 10 ... after the final outputs are produced"). If a future BMAD revision drops `on_complete` from any of the four skills, the templates below need a different hook.

Use `persistent_facts` for the rules registry (informational). Use `on_complete` for behaviors that must fire at completion (the structured-completion handshake, final-stage verification gates).

### File 1 — `_bmad/custom/<project>-skill-rules.md`

The cross-cutting rule registry. Loaded by every skill via `persistent_facts`. Add project-specific rules below the seven universals as your project's retrospectives surface them.
...
# Loads <project>-skill-rules.md as persistent facts (informational) AND
...
# See _bmad/custom/<project>-skill-rules.md for the rule set.
#
...
# Loads <project>-skill-rules.md as persistent facts (informational) AND
# attaches an on_complete hook that fires at the workflow's terminal stage
# for the test-discoverability check and the structured-completion handshake.
# See _bmad/custom/<project>-skill-rules.md for the rule set.
...
on_complete = [
  """
  Verify the test files you generated are actually discoverable by the project's
  default test suite:

  - Confirm the new test files follow the project's test-file naming convention
    (whatever extensions, prefixes, or directory placements the project uses).
  - Confirm they are NOT excluded by an ignore file (e.g., .testignore,
    .gitignore patterns the runner respects).
  - Confirm they are NOT tagged or marked in a way that excludes them from the
    default run (e.g., slow / integration / @skip markers that the default suite
    explicitly opts out of).

  The purpose of QA is *new tests that run* — not new tests that exist on disk
  but are silently skipped. If any new test file would be skipped by default,
  either re-tag it, move it, or surface the issue to the lead.
  """,
  """
  If invoked as a subagent under /epic-cycle (the calling context will include
  the literal phrase "Single-Task Agent (Task-in-Prompt Mode)" in the spawn
  prompt), follow Rule 4: send EXACTLY ONE STATUS: completed envelope to the
  team lead in the format specified in the rules file, then STOP.

  Populate TESTS_ADDED with the full paths of every test file you created
  (relative to repo root). Populate DECISIONS with one-line summaries of any
  non-obvious test-design choices. Populate ISSUES_ENCOUNTERED with anything
  you had to surface (e.g., a story without enough AC detail to test, a
  testability gap in the dev work, a discoverability issue you fixed).
...
# Loads <project>-skill-rules.md as persistent facts (informational) AND
# attaches an on_complete hook that fires at the workflow's terminal stage
# to enforce ADR-violation severity, Integration AC verification, deferred-work
# tracking, and the structured-completion handshake.
# See _bmad/custom/<project>-skill-rules.md for the rule set.
...
on_complete = [
  """
  Verify the review you produced complies with the following rules from
  <project>-skill-rules.md:

  - Rule 3 (per-story smoke evidence): if this story touches a user-facing
    surface and the review approves it without evidence of a smoke in the
    target runtime, that is a HIGH finding. File it.
  ...
  - Rule 6 (ADR violations are HIGH): for each AC in the story whose
    implementation choice is constrained by an Accepted ADR, verify the
    implementation matches the ADR's commitment. Any mismatch is HIGH severity,
    NOT a LOW deferrable. File it.
  - Rule 1 (Integration ACs): if this is a service-introducing story and no
    ...
    (the producer would ship with consumer wire-up untested). File it.

  All deferred items (any item not auto-resolved during this review) MUST be
  added to _bmad-output/implementation-artifacts/deferred-work.md with the
  originating story ID, severity, issue summary, deferral rationale, and
  suggested resolution. Deferring to the story file alone is insufficient —
  centralized tracking is what makes deferred items visible at the next epic's
  Story X.0 triage.
  ...
  Populate DECISIONS with the resolved/deferred/dismissed counts and any HIGH
  fix names (e.g., "resolved 2 HIGH: integration-AC missing for X; ADR-0010
  violation in Y; deferred 5 LOW; dismissed 1"). Populate ISSUES_ENCOUNTERED
  with anything unusual (e.g., NFR tripwire surfaced and amendment filed).
...
2. **All five customization files exist:**
   ...
   ls _bmad/custom/
   ```
   Expected entries: `<project>-skill-rules.md`, `bmad-create-story.toml`, `bmad-dev-story.toml`, `bmad-qa-generate-e2e-tests.toml`, `bmad-code-review.toml`. (Plus whatever else BMAD installs alongside; ignore non-listed files.)

3. **Each `.toml` references `on_complete`:**
   ```
