...
Five epics (Epic 0 foundation + Epics 1–5) have shipped. The visitor-facing simulation is complete end-to-end across the full mission window (1977-08-20 through 2030-12-31) — every chapter (launches, six gas-giant encounters, Pale Blue Dot, both heliopause crossings) is renderable, with the PBD hero scene's choreographed turn + photo-plate compositing operational. Epic 6 (audio + reduced-motion + accessibility + launch-gate qualitative testing) and Epic 7 (operational substrate + launch readiness) remain.
...
| Epic 6 — Audio, Reduced Motion, Accessibility | 📋 backlog | Golden Record audio bundle + `<v-audio-toggle>` activating gently at launch / PBD / heliopause; `<v-hud>` dismiss/restore via `H` + narrow-viewport HUD compaction; full reduced-motion sweep across every chapter and component (all transitions → 0 ms under `prefers-reduced-motion: reduce`); axe-core CI expansion + manual a11y test checklist (keyboard-only, VoiceOver, NVDA, color-blindness, forced-colors, photosensitive-epilepsy audit); friendly-user qualitative testing (5–10 first-time users + ≥1 AT user — **launch gate**: ≥50% must perceive the attitude reconstruction unprompted at V1 Jupiter); final contrast / typography / provenance-label polish + reference-parity review (≥2 of 3 external reviewers must say "linkable without apology"). |
...
**FR closures achieved through Epic 5:** FR1 (sub-day scrub during encounters), FR4 + FR37 + FR41 (deep-link URL contract), FR8–FR12 (gravity-assist visible + spacecraft model rendering), FR15–FR20 (attitude reconstruction operational), FR21–FR26 (V1J / V2J / V1S / V2S / V2U / V2N encounter chapters), FR27 (V1 physically turns toward inner solar system at PBD), FR28 (NASA photo plates composite at historical instants), FR30 (PBD chapter copy + six gas-giant encounters with hand-written prose), FR33 (single-action restore-default camera), FR34 (HUD: chapter title, distance, attitude provenance, speed multiplier), FR55 (L4 visual regression operational — 14 baselines covering 8 encounter scenes + 4 PBD substates + 2 production-build HUD-title regressions). AR10 (PBD as dedicated `ChapterModule` per ADR-0014) operational.

**Test pyramid baseline at Epic 5 close (post-cross-review):** web vitest **3343** passed / 10 skipped / 186 files (+255 from post-Epic-4 baseline); bake fast-tier pytest 430 / 4 / 24 (unchanged — Epic 5 is web-side); L4 Playwright **15** tests / ~1.2-minute wall-clock (12 chapter baselines + 2 production-HUD-title regressions + 1 SCENES-roster invariant), deterministic across 3+ consecutive reruns post-BUG-007 fix; bake slow-tier (LFS kernels gated) 5+ moon-trajectory E2E tests; typecheck clean; lint baseline preserved (4 warnings, 0 errors).

Per-epic retrospectives are saved alongside the cycle logs under `_bmad-output/implementation-artifacts/epic-N-retro-<date>.md`; see Epic 5's at [`epic-5-retro-2026-05-24.md`](_bmad-output/implementation-artifacts/epic-5-retro-2026-05-24.md) for the cross-review lessons that carry into Epic 6 (chief among them: per-story smoke gates assert on text not layout, and L4 baselines captured against a broken state pass against themselves — Epic 6 / Story 6.6 polish will close that gap with a LAYOUT-asserting production-build regression test).
...
Planning + five implementation epics complete; Epic 6 (audio + reduced-motion + accessibility + launch-gate qualitative testing) is next.
...
- 📋 **Epic 6 — Audio, Reduced Motion, Accessibility.** Golden Record audio + `<v-audio-toggle>` (off by default, gentle at launch/PBD/heliopause); HUD dismiss/restore + narrow-viewport compaction; full reduced-motion sweep; axe-core CI expansion + manual a11y checklist (VoiceOver / NVDA / forced-colors / color-blindness / photosensitive-epilepsy); **friendly-user qualitative testing as launch gate** (5–10 users + ≥1 AT user; ≥50% must perceive the attitude reconstruction unprompted at V1 Jupiter); final contrast / typography / provenance-label polish + external reference-parity review.
- 📋 **Epic 7 — Operational Substrate + Launch Readiness.** Drift report tool + path-filtered kernel update workflow; L5 Playwright mission-timeline E2E assertion; build-manifest observability; CSP + deploy-rollback rehearsal; final performance pass; cross-browser + real-device testing; 25 ADR completion audit; public launch playbook.
