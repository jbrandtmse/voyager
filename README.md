...
> 🚀 **Epics 1–6 complete; Epic 7 (operational substrate + launch readiness) is next.** All six gas-giant encounter chapters (V1J / V2J / V1S / V2S / V2U / V2N), the Pale Blue Dot hero scene, and both heliopause crossings are operational end-to-end across the full 53-year mission window. The Voyager Golden Record audio layer activates gently at launch / PBD / heliopause; the HUD compacts at narrow viewports; reduced motion is honored across every transition; axe-core a11y is wired into CI. A two-pass cross-review at the close of Epic 6 surfaced 15 + 4 defects across the full mission lifecycle — all 19 fixed, including the Pale Blue Dot photo plates now rendering as celestial moments rather than rectangular polaroids. See [Implementation Status](#implementation-status) for the per-epic shipping summary.
...
Six epics (Epic 0 foundation + Epics 1–6) have shipped. The visitor-facing simulation is complete end-to-end across the full mission window (1977-08-20 through 2030-12-31) — every chapter (launches, six gas-giant encounters, Pale Blue Dot, both heliopause crossings) is renderable, with the PBD hero scene's choreographed turn + photo-plate compositing operational. Golden Record audio activates at the launch / PBD / heliopause chapter markers; the HUD compacts gracefully at narrow viewports; reduced-motion is honored globally; axe-core a11y is in CI. Only Epic 7 (operational substrate + launch readiness) remains.
...
| Epic 5 — Pale Blue Dot | ✅ done | Dedicated `PaleBlueDot` chapter module (per [ADR-0014](docs/adr/0014-hybrid-chapter-definition-spec-for-10-module-for-pbd.md)) at `web/src/chapters/pale-blue-dot/` with an 11-substate machine (idle → turning → 6× sweeping_&lt;body&gt; → composite_active → composite_decay → passed) anchored to the 1990-02-14 imaging sequence (180s cinematic arc, ~100× from the historical ~5-hour sequence); choreographed turn with mixed CK+synthesized coverage (V1 bus = CK from `vgr1_super_v2.bc`; scan platform = PBD-module-synthesized per-substate aim using `setFromUnitVectors([0,0,1], busInverse · V1→target_J2000)` math — Path A override-injection into `AttitudeApplier`); 6 NASA Photojournal photo plates (PIA00452 canonical PBD Earth + PIA00453 family-portrait six-panel grid cropped per body) composite at each substate peak via HTML overlay (`<div class="pbd-composite-layer">`) with SLERP-with-ease-out transitions + 30-second Earth-pause hold during `composite_active`; 4 PBD L4 Playwright visual-regression baselines (`pbd-turning`, `pbd-sweeping-earth`, `pbd-sweeping-neptune`, `pbd-composite-decay`) with cross-check assertions against `PBD_SUBSTATE_TIMINGS` for sub-second timing-drift feedback; Story 5.0 cleanup audited Epic 3 retro action items + closed Epic 4 retro addendum items (BUG-006 dist-drift investigation + LFS-size policy in [`CONTRIBUTING.md`](CONTRIBUTING.md)); cross-review surfaced + fixed [BUG-E5-007](`_bmad-output/implementation-artifacts/bug-reports-epic5-2026-05-24/BUG-E5-007-production-css-link-missing.md`) (CRITICAL — production HTML never linked the main CSS file because Story 1.8's capability probe replaced the static script tag with a dynamic import, bypassing Vite's auto-CSS-injection) + L4 stable-frame Condition 5 (HTML composite overlay opacity-settle gate) + BUG-E5-008 (HUD top-right icon overlap) + BUG-E5-009 (bottom-row scrubber gutters cleared from play button + speed multiplier readout). |
| Epic 6 — Audio, Reduced Motion, Accessibility | ✅ done | 5 NASA Golden Record audio tracks (Carter's English greeting, Arabic greeting, Wind/Surf, Life Signs/Pulsar, Music of the Spheres) wired through `<v-audio-toggle>` with fade-in/out at the launch / PBD / heliopause markers (Story 6.1; single-play per BUG-CR-008 — no loop); `H` dismisses/restores HUD, narrow-viewport (< 1024 px) HUD compaction collapses distance + instruments under a `⋯` button and routes chapter copy through a bottom-sheet drawer with grab-handle (Story 6.2); full reduced-motion sweep — every motion-duration token (`--v-duration-fast/-base/-slow`) collapses to `0ms` at `:root` under `prefers-reduced-motion: reduce`, audited document at [`docs/accessibility/reduced-motion.md`](docs/accessibility/reduced-motion.md) (Story 6.3); axe-core in CI + manual a11y checklist + 3 critical a11y fixes (Story 6.4); friendly-user qualitative testing protocol + Rule 17 launch-gate at [`docs/launch/friendly-user-testing-protocol.md`](docs/launch/) (Story 6.5); final contrast / typography / provenance-label polish + L4 mask tightening (Story 6.6). Two cross-reviews at epic close surfaced **19 defects** (15 in the initial review + 4 from the post-fix end-to-end walkthrough); all 19 fixed before merge — including the Pale Blue Dot photo plates now compositing as a scattered sunbeam over the starfield rather than a hard rectangle (bake-time luminance-threshold alpha mask), spacecraft + moons perceivable at encounter framing via dynamic scale exaggeration (1,000,000× spacecraft / 8× moons lerped by ViewFrame encounter-alpha), Saturn rings rendered as procedural geometry tilted to Saturn's 26.73° obliquity in the J2000 Z-up convention, and a WebGL context-loss / restore handler so transient GPU drops auto-recover. Bug logs at [`epic-6-cross-review-bugs.md`](_bmad-output/implementation-artifacts/epic-6-cross-review-bugs.md) + [`epic-1-6-e2e-review-bugs.md`](_bmad-output/implementation-artifacts/epic-1-6-e2e-review-bugs.md). |
| Epic 7 — Operational Substrate + Launch Readiness | 📋 backlog | Drift report tool + path-filtered kernel update workflow; L2 trajectory consistency + L5 Playwright E2E mission-timeline assertion; build-manifest observability + dependency-advisory checks; content-hashed immutable headers + CSP + deploy-rollback rehearsal; THIRD_PARTY.md + MISSION_FACTS.md final audit; final performance pass on mid-range laptop; cross-browser + real-device Tier 1/2/3 testing; 25 ADR completion audit + index cleanup; public launch playbook + pre-flight gate. |

**FR closures achieved through Epic 6:** FR1 (sub-day scrub during encounters), FR4 + FR37 + FR41 (deep-link URL contract), FR8–FR12 (gravity-assist visible + spacecraft model rendering), FR15–FR20 (attitude reconstruction operational), FR21–FR26 (V1J / V2J / V1S / V2S / V2U / V2N encounter chapters), FR27 (V1 physically turns toward inner solar system at PBD), FR28 (NASA photo plates composite at historical instants), FR30 (PBD chapter copy + six gas-giant encounters with hand-written prose), FR33 (single-action restore-default camera), FR34 (HUD: chapter title, distance, attitude provenance, speed multiplier), FR46 (Golden Record audio toggleable diegetic layer), FR47 (`<v-hud>` dismiss/restore), FR48 (narrow-viewport HUD compaction), FR55 (L4 visual regression operational — 14 baselines covering 8 encounter scenes + 4 PBD substates + 2 production-build HUD-title regressions), NFR-A1 → A6 (WCAG 2.2 AA accessibility floor — keyboard operability, screen-reader landmarks, focus management, color-contrast audit, reduced-motion, photosensitive-epilepsy safe). AR10 (PBD as dedicated `ChapterModule` per ADR-0014) operational. AR13 (MissionPhaseFSM SOI crossings) operational.

**Test pyramid baseline at Epic 6 close (post-cross-review + post-E2E-review):** web vitest **3875** passed / 10 skipped / 223 files (+532 from Epic 5 close baseline); bake fast-tier pytest 430 / 4 / 24 (unchanged — Epic 6 was web-side); L4 Playwright 15 tests / ~1.2-minute wall-clock (deterministic across 3+ consecutive reruns); bake slow-tier (LFS kernels gated) 5+ moon-trajectory E2E tests; typecheck clean; lint baseline preserved (4 warnings, 0 errors).

Per-epic retrospectives are saved alongside the cycle logs under `_bmad-output/implementation-artifacts/epic-N-retro-<date>.md`; see Epic 6's at [`epic-6-retro-2026-05-25.md`](_bmad-output/implementation-artifacts/epic-6-retro-2026-05-25.md) for the synthesis of interactive friendly-user testing prep + the Rule 18 candidate (the cross-review pattern: a maintainer + agent walk through the full deployed surface chapter-by-chapter looking for defects that pixel-diff regression can't catch). Cross-review bug logs at [`epic-6-cross-review-bugs.md`](_bmad-output/implementation-artifacts/epic-6-cross-review-bugs.md) (15 bugs, all fixed) and [`epic-1-6-e2e-review-bugs.md`](_bmad-output/implementation-artifacts/epic-1-6-e2e-review-bugs.md) (4 bugs from the post-fix walkthrough; 1 fixed at review time, 3 pending Epic 7 follow-up).
...
Planning + six implementation epics complete; Epic 7 (operational substrate + launch readiness) is the final epic before public launch.
...
- ✅ **Epic 5 — Pale Blue Dot.** Dedicated `PaleBlueDot` chapter module + 11-substate machine; choreographed spacecraft turn (V1 bus CK + scan platform PBD-synthesized per-target aim); 6 NASA Photojournal photo plates composite at substate peaks with 30-second Earth-pause hold; 4 PBD L4 Playwright baselines. Cross-review surfaced + fixed 4 layout-class bugs (production CSS link, L4 stable-frame condition for HTML overlays, HUD top-right icon overlap, scrubber gutters). See [Implementation Status](#implementation-status) for the full per-story summary.
- ✅ **Epic 6 — Audio, Reduced Motion, Accessibility.** Golden Record audio + `<v-audio-toggle>` (off by default, gentle at launch/PBD/heliopause; single-play, no loop); HUD dismiss/restore + narrow-viewport compaction (bottom-sheet drawer for chapter copy); full reduced-motion sweep; axe-core CI expansion + manual a11y checklist + 3 critical a11y fixes; friendly-user qualitative testing protocol + Rule 17 launch-gate; final contrast / typography / provenance-label polish + L4 mask tightening. **Two-pass cross-review** at epic close surfaced 19 defects across the full mission lifecycle; all 19 fixed before merge — including PBD photo plates now compositing as a scattered sunbeam rather than a rectangular polaroid (bake-time luminance-threshold alpha mask), spacecraft + moons perceivable at encounter framing (1,000,000× spacecraft / 8× moons dynamic scale exaggeration lerped by ViewFrame encounter-alpha), Saturn rings rendered procedurally at 26.73° obliquity, HUD distance flicker eliminated (LRU capacity bumped 12 → 64), heliocentric system-view `V` keyboard shortcut + help overlay entry, WebGL context-loss handler. See [Implementation Status](#implementation-status) for the full per-story summary.
- 📋 **Epic 7 — Operational Substrate + Launch Readiness.** Drift report tool + path-filtered kernel update workflow; L5 Playwright mission-timeline E2E assertion; build-manifest observability; CSP + deploy-rollback rehearsal; final performance pass; cross-browser + real-device testing; 25 ADR completion audit; public launch playbook. **Three deferred bugs from the Epic 6 E2E review** carry into Epic 7: launch-chapter camera at world origin (E2E-001 HIGH), PBD cold-load shows no plate until Play (E2E-003 LOW), heliopause + V2 Neptune chapter copy overlaps HUD top-right (E2E-004 HIGH).
