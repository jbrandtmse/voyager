...
- **Storage formats at runtime:** Custom 40-byte-header VTRJ binary (raw little-endian Float64Array), brotli-compressed. Decompression is performed client-side by the `brotli-dec-wasm` polyfill (Story 1.16; originally planned to use `DecompressionStream('br')` but that JS API never standardized brotli — see ADR 0004 § Decompression Strategy). **Body layout is discriminated by `body_id` namespace** (Story 3.1 amendment): trajectory body IDs `{-31, -32, 10, 1..8, 301}` store `[x,y,z,vx,vy,vz]` at uniform cadence (time implicit via `et_start + i × cadence_seconds`); attitude body IDs `{-31000, -31100, -32000, -32100}` store `[et, qw, qx, qy, qz]` with explicit per-sample ET to support the mission's variable-cadence schedule. JSON/CSV/MessagePack/Protobuf/Arrow/Parquet at runtime all explicitly rejected.
...
- Custom VTRJ binary (40-byte header + raw little-endian Float64Array body), brotli-compressed for transport; client decompression via `brotli-dec-wasm` polyfill (Story 1.16). Body layout per `body_id` namespace: trajectory = `(N, 6)` `[x,y,z,vx,vy,vz]` uniform cadence; attitude = `(N, 5)` `[et, qw, qx, qy, qz]` explicit per-sample ET (Story 3.1 amendment — see ADR 0004 § Body Layout per Kind)
- Chunking: per-decade trajectory files at daily cadence; per-encounter higher-cadence overlays (1-hour at ±30 days from CA, 1-minute at ±2 days, 10-second at ±1 hour). For attitude, the mixed cadence is stored inline in a single file per encounter-kind (explicit-ET body); for trajectory, mixed cadence requires separate files per band (implicit-ET body)
