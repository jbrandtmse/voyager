...
**And** the grid cadence matches the encounter cadence schedule. **Amended 2026-05-21 (Story 3.1 slow-tier calibration):** uniform 5-second cadence across closest_approach ±2 days, single band per encounter file. The original wording ("1-minute during flyby ±2 days; 10-second during closest approach ±1 hour; daily during CK-covered cruise segments") was inadequate — real CK content at V2 Uranus Miranda imaging (~4 hr pre-CA) drives the scan platform faster than 1 mrad/min, so the outer 1-min band breached NFR-P10 by 32× when first measured against real kernels. Cruise-period attitude is synthesized (Story 3.2 HGA-Earth-pointing), NOT baked from CK, so daily cruise samples are correctly excluded from encounter files. **Amended 2026-05-22 (Story 4.0 hotfix — CI-RED tripwire closure):** variable cadence — **1-second inside closest_approach ±4 hours** (HALF_WIDTH_1S band) **and 5-second across closest_approach ±2 days** (HALF_WIDTH_1MIN band) with the inner ±4hr band masked out so consecutive samples are never duplicated. The 2026-05-21 uniform 5-sec calibration was tuned against V2 Uranus peak slew only; Story 3.7's L2 fixture surfaced V2 Saturn active imaging at ET ~-579086636 (CA + 2.59 hours) with neighboring 5-sec knots reporting 5.8 mrad pairwise angular delta → SLERP mid-interval = 3.6 mrad, 3.6× over the NFR-P10 1 mrad gate. Initial Story 4.0 implementation used a ±1 hour inner band; AC8 smoke surfaced the same V2 Saturn ET still breaching at 3.3 mrad because the failing ET is at CA + 2.6 hours — outside ±1 hour but inside the active-imaging window. The inner band was widened to ±4 hours to cover the documented V2 Saturn imaging sequence. The 1-sec inner band reduces SLERP error by ~25× inside active-imaging windows (error scales as cadence² × angular acceleration); the outer 5-sec band preserves the Story 3.1 V2-Uranus-margin elsewhere. Path (c) variable-cadence-around-CA from the deferred-work item `[3.7 / HIGH]` #1 (rejected option (d) — relax NFR-P10 — per user decision 2026-05-22). For type-1 PDS Rings ISS SEDR scan-platform CKs (zero-duration `(t, t)` intervals per shutter event), the bake now emits explicit-ET `platform_attitude` VTRJs containing the discrete in-band knot ETs (AC2 / `[3.7 / HIGH]` #2). See voyager-skill-rules.md Rule 5 (planning artifacts amended in place) and ADR-0004 § Body Layout per Kind for the canonical schema.,
