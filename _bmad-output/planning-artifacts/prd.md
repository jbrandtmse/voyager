...
- **Attitude accuracy in encounter windows.** Spacecraft and scan-platform quaternions interpolated from CK kernels match the source within **≤0.05° (≤1 milliradian, ≤100 NA-pixel error)**. Synthesized Earth-pointing HGA cruise attitude is clearly UI-labeled as synthesized — not silently substituted for CK data.
...
- **NFR-M4.** Test execution time: the full L1+L2+L3 test suite (Python validation, JS-vs-SPICE consistency, TS unit tests) completes in ≤ 5 minutes on a standard CI runner. The L4+L5 suite (Playwright visual regression + E2E timeline) completes in ≤ 15 minutes.
